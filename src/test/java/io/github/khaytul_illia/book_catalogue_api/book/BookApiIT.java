...
public class BookApiIT {
        ...
        void shouldGetAPageOfBooks(){
            ...
            restClient
                ...
                .expectBody(responseType).value(response -> {
                    ...
                    assertThat(response.content().stream().map(BookResponse::id))
                        .containsAnyElementsOf(books.stream().map(Book::getId).toList());
                });
        }
    ...
    class ErrorPathTests{
        ...
        @Test
        @DisplayName("Should return 400 Bad Request when invalid request parameter")
        void shouldReturn400_whenInvalidRequestParameter(){
            //Arrange
            long bookId = -1;
            BookUpdateRequest request = new BookUpdateRequest(null, null, null, 1, null);
            ...
            restClient
                ...
                .expectBody(ErrorResponse.class).value(response -> {
                    ...
                    assertThat(response.data().get("bookId")).isEqualTo("must be greater than 0");
                });
        }
        ...
        @Test
        @DisplayName("Should return 401 Unauthorized when accessing with wrong credentials")
        void shouldReturn401_whenBadCredentials(){
            //Arrange
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.APPLICATION_JSON);
            headers.setBasicAuth(username, "wrong_" + password);
